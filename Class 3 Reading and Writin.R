...
input_csv.df <- read.csv(file="/Users/stacy/Downloads/binary.csv")
...
##rounding of number
##round function
##To remove a colum there are two ways
student <- student[,c(2:6)]
student <- student[,-1]
#Names of column
names(student)
...
csv_file <- read.table(file="/Users/stacy/Downloads/binary.csv",
                                header = TRUE, 
                                sep = ',')

## Read a Tab Delimited file
input_tabdlmtd.df <- read.table(file="/Users/stacy/Downloads/tab_delimited_data.txt",
                                header = TRUE, 
                                sep = '\t')
...
input_dollar.df <- read.table(file="/Users/stacy/Downloads/dollar_delimited_data.txt",
                                header = TRUE, 
                                sep = '$')
...
bank_ins = read.sas7bdat("/Users/stacy/Downloads/bank_additional_full.sas7bdat")
names(bank_ins)
...
accessToken <-"EAACEdEose0cBAPiO9XaZCYZB8nskwsQr0oOUcFnVoMrsqfk8aBx1zxBvzvfIqOMK4BpA3yJwjqgiMZCejk6vC1LTgQ1CHmYSX2CoIWGpZBU3hEFZBi4lxw8aWGVumsZBg4LeF8JUO8Av0JKGZCZAkh1bJ58G6QzLDH0t53zSMPAoQuo0PzX1HEbgtTbSDijPREgyamZAsTEvZCGN4XdO71vMep4RwNHq1T5YZB0hkckiaCBwAZDZD"
...
matchesURL <- getURL("https://en.wikipedia.org/wiki/India–Pakistan_cricket_rivalry")
library(XML)
WikiPage<-readHTMLTable(matchesURL,header=T)
View(WikiPage)
...
write.csv(tt, "/Users/stacy/Downloads/mtcars.csv",
          row.names=F
)
...
class_demo$exp_Age <- exp(class_demo$age)
class_demo$log_Age <- log(class_demo$age)
class_demo$inv_Age <- 1/(class_demo$age)
class_demo$sqrt_Age <- sqrt(class_demo$age)
class_demo$square_Age <- (class_demo$age)*(class_demo$age)
...
write.csv(students,
          ...
          row.names = F)
getwd()
#--------------------------------------------------------------------- Connect to facebook and extract data


#Filter condition in a data frame
student1 <- class_demo$age>=23
student4 <- class_demo$age[student1,]
student5 <- class_demo[class_demo$age>=23 & class_demo$C>=99,]
student5 <- class_demo[class_demo$age>=23 & class_demo$C>=99,c(1:2)]

#Sample function
sample_index <- sample(1:nrow(class_demo),3,replace=F)
sample_index
student6 <- class_demo[sample_index,]
student6
subset #function can also be used to selectively choose data from a data frame

#Appending data fram vertically
...
k <- read.table("https://data.oecd.org/healthstat/infant-mortality-rates.htm")

read.table("https://en.wikipedia.org/wiki/India%E2%80%93Pakistan_cricket_rivalry")
